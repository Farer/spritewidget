...
class MotionTween<T> extends MotionInterval {
  ...
  void update(double t) {
    ...
    if (startVal is Offset) {
      ...
      // Colors
      int aNew = int.parse( (((startVal as Color).alpha + (_delta.alpha * t).toInt()).clamp(0, 255)).toString() );
      int rNew = int.parse( ((startVal as Color).red + (_delta.red * t).toInt()).clamp(0, 255).toString() );
      int gNew = int.parse( ((startVal as Color).green + (_delta.green * t).toInt()).clamp(0, 255).toString() );
      int bNew = int.parse( ((startVal as Color).blue + (_delta.blue * t).toInt()).clamp(0, 255).toString() );
